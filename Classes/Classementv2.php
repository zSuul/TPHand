...
Class Classementv2 {
	...
	public function compare($a, $b){ // Fonction "compare" pour la question 4 b)
	    ...
	    if($a->points > $b->points)
	    	...
	    	foreach ($this->lesJournees as $uneJournee) {
		    	if($a->nom == $uneJournee->domicile && $b->nom == $uneJournee->exterieur){
		    		if($uneJournee->scoreDomicile > $uneJournee->scoreExterieur)
	            		return -1 ;
	       			else if($uneJournee->scoreDomicile > $uneJournee->scoreExterieur)
	            		return 1 ;
		    	}
		    	else if($a->diff > $b->diff)
		            return -1 ;
